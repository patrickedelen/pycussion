...
class Lighting:
    ...
    def mode_wash(self):
        if self.cur_lighting_mode != 'wash':
            ...
            self.active_intensity = 50
        
        if self.wash_rising and self.active_intensity < 250:
            self.active_intensity += 5
        elif self.wash_rising and self.active_intensity >= 250:
            self.wash_rising = False
            self.active_intensity -= 5
        elif not self.wash_rising and self.active_intensity > 50:
            self.active_intensity -= 5
        else:
            self.wash_rising = True
            self.active_intensity += 5
    ...
    def render(self, magnitude, close, switch=False, lighting_state={}):
